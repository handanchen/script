...
function execute()
{
    execute_meta "select id,table_name,src_db_id,tar_db_id from t_task where status = 0 and valid = 1 LIMIT 20;" | while read task_id table_name src_db_id tar_db_id; do
        ...
        begin_time=`date +%s`
        # 启动任务
        sh task_runner.sh $task_id 2>&1 | tee -a $TMP_PATH/test.log
        # 结束时间
        end_time=`date +%s`
        ...
        # 日志
        echo -e "${src_db[0]}_${src_db[1]}.${src_db[2]}\t${tar_db[0]}_${tar_db[1]}.${tar_db[2]}\t${table_name}\t$((end_time-begin_time))\t${file_size}" | tee -a $TMP_PATH/task.log
    done
}
...
{
    # 获取sqoop耗时
    grep -iE "mapreduce.ImportJobBase: Transferred" $TMP_PATH/test.log |
    sed 's/.* Transferred \(.*\) in \(.*\) seconds (\(.*\))/\1\t\2\t\3/i;s/,//g' > $TMP_PATH/sqoop.log

    paste -d '\t' $TMP_PATH/task.log $TMP_PATH/sqoop.log |
