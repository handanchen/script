...
{
    if [ $# -gt 0 ]; then
        echo -e "$@" >> $TMP_PATH/$src_db_name/$table_name.opts
    else
        cat >> $TMP_PATH/$src_db_name/$table_name.opts
    fi
}
...
function execute()
{
    ...
    if [ "${src_db_type}" = "HIVE" ]; then
        ...
        if [ "${tar_db_type}" = "FS" ]; then
            # 导出到本地文件系统
            local export_dir=$DATA_PATH/$src_db_name/$table_name
            ...
            echo "export" > $TMP_PATH/$src_db_name/$table_name.opts
        ...
        echo "import" > $TMP_PATH/$src_db_name/$table_name.opts
        ...
        if [ $query_sql -eq 1 ]; then
            ...
            sed -i 's/\\\*/\*/g' $TMP_PATH/$src_db_name/$table_name.opts
    ...
    config_sqoop "--bindir \n $TMP_PATH/$src_db_name"
    config_sqoop "--outdir \n $TMP_PATH/$src_db_name"

    # 日志
    log `cat $TMP_PATH/$src_db_name/$table_name.opts`

    sqoop --options-file $TMP_PATH/$src_db_name/$table_name.opts
